...
interface AssetLocationManagerProps {
  projectId: number;
}

// Default type for backend compatibility - type is required but not used in the UI
const DEFAULT_LOCATION_TYPE = "area";

export default function AssetLocationManager({ projectId }: AssetLocationManagerProps) {
  const { toast } = useToast();
  const [createDialogOpen, setCreateDialogOpen] = useState(false);
  const [editDialogOpen, setEditDialogOpen] = useState(false);
  const [selectedParentId, setSelectedParentId] = useState<number | null>(null);
  const [selectedAssetLocation, setSelectedAssetLocation] = useState<AssetLocation | null>(null);
  ...
  const { 
    data: assetLocations = [], 
    isLoading: isAssetLocationsLoading, 
    error: assetLocationsError,
    refetch: refetchAssetLocations
  ...
  const handleCreateAssetLocation = () => {
    ...
    // Create the asset location data - type is required by backend but not meaningful in UI
    const locationData = {
  ...
  const handleUpdateAssetLocation = () => {
    ...
    updateAssetLocationMutation.mutate({
      ...
      data: {
        name: assetLocationName,
        // Keep the existing type, but fall back to default if missing
  ...
  const buildAssetLocationTree = (items: AssetLocation[], parentId: number | null = null): AssetLocation[] => {
    return items
  ...
  const renderAssetLocationNode = (node: AssetLocation & { children?: AssetLocation[] }, level = 0) => {
      ...
      <div key={node.id} className="asset-location-tree-node">
        ...
        {isExpanded && hasChildren && node.children && (
          <div className="asset-location-tree-children">
            {node.children.map((child: AssetLocation) => 
              renderAssetLocationNode(child, level + 1)
            )}
          </div>
        )}
      </div>
  ...
  const assetLocationTree = buildAssetLocationTree(assetLocations);
  const hasAssetLocations = assetLocations.length > 0;
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Add a specific location where building assets are physically located
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            {project && (
              <div className="bg-blue-50 p-3 rounded-md text-sm">
                ...
                <p className="text-xs text-gray-500 mt-1 ml-6">
                  Adding a new asset location to this project
                </p>
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Enter a descriptive name for where assets are physically located.
              </p>
            </div>
            ...
            <div className="bg-blue-50 p-3 rounded-md mt-4">
              ...
              <p className="text-xs text-gray-700 mt-1 ml-6">
                When selecting assets in the NRM3 Framework tab, you'll assign them to these asset locations.
              </p>
            </div>
          </div>
